...
int XFFmpeg::Open(const char *path)
{
    ...
    for (int i = 0; i < ic->nb_streams; i++)
    {
        ...
        else if(enc->codec_type == AVMEDIA_TYPE_AUDIO)
        {
            ...
            printf("audio sampleRate: %d sampleSize: %d channnel: %d\n",this->samplleRate,this->sampleSize,this->channel);
        }
    }
    mutex.unlock();
    return totalMs;
...
void XFFmpeg::offset(int64_t stamp){
    while(yuv!=NULL&&yuv->pts>stamp){
        AVPacket pkt = Read();
        if(pkt.size<=0)
        {
            continue;
